...
set "PROGRAMS_FOLDER=%DIR_ATUAL%Programs"
set "LOGFILES=%TMP%\tmp"
...
REM ----------- SCRIPT VRS 9.4 ------------
set version=9.4
set "dllsitef_vrs=7.0.117.112.r3"
set "dllsitef_vrs_old=7.0.117.108.r1"

REM -- Inicia Menu
call :checkProgramsFolder
goto menu
...
call :checkjClisitef &
call :checkProgramsFolder_deleteOlderFiles &
call :deleteTempConfirmationFiles &
...
echo * 12. RustDesk
echo * 13. Banco_PDV
echo * 14. Configurar NFCe vr.properties
echo * -------------------------------------
echo * 15. CheckList PDV
echo =====================================
...
if %opcao% equ 14 cls & call :settingVRProperties_NFCe & goto menu
if %opcao% equ 15 goto checklistPdvMenu
if %opcao% GEQ 16 call :opcaoinexistente menu
...
if %opcaoJava% equ 3 call :javaVersions
if %opcaoJava% equ 4 call :javaUninstaller & pause & goto menu
...
if %opcaoJavavrs% equ 2 call :updateJavaVRS "%java251link%" 251 & pause & goto menu
if %opcaoJavavrs% equ 3 call :updateJavaVRS "%java291link%" 291 & pause  & goto menu
if %opcaoJavavrs% equ 4 call :updateJavaVRS "%java301link%" 301 & pause  & goto menu
if %opcaoJavavrs% equ 5 call :updateJavaVRS "%javaXXXlink%" Latest & pause  & goto menu
...
if exist "C:\vr\vr.properties" (
call :UpdateOrAddTokenTLS "C:\vr\vr.properties" "%tokenVPNSitef%"
call :ConfirmTokenInserted "C:\vr\vr.properties" "%tokenVPNSitef%"
)
if exist "%userprofile%\.vr\vr.properties" (
call :UpdateOrAddTokenTLS "%userprofile%\.vr\vr.properties" "%tokenVPNSitef%"
call :ConfirmTokenInserted "%userprofile%\.vr\vr.properties" "%tokenVPNSitef%"
)
goto continueTLS
...
del /q "%file%.tmp" >nul 2>&1
exit /b
REM #################### UpdateOrAddTokenTLS ####################
REM #################### ConfirmTokenInserted ####################
:ConfirmTokenInserted
set "file=%~1"
...
for /f "usebackq tokens=1,* delims==" %%A in ("%file%") do (
    if "%%A"=="tokentls" (
        if "%%B"=="%expected_value%" (
            call :registerLog "TLS_PROPERTIES" "SUCCESS"
        ) else (
			call :registerLog "TLS_PROPERTIES" "FAILED"
		)
    )
)
...
if exist C:\VRPdv\libs (
  ...
  call :registerLog "TLS_LIBS" "SUCCESS"
)
...
call :updateJavaVRS "%java301link%" 301
call :registerLog "UPDATE_JAVA" "SUCCESS"
)

cls
echo:

call :searchInFile "%LOGFILES%\UPDATE_DLLSITEF_SYSWOW64.txt" "SUCCESS" && echo: && echo Dlls Sitef atualizadas com sucesso em C:\Windows\SysWOW64
call :searchInFile "%LOGFILES%\TLS_SYSWOW64.txt" "SUCCESS" && echo Arquivo CONFITLS.ini configurado com sucesso em C:\Windows\SysWOW64

call :searchInFile "%LOGFILES%\UPDATE_DLLSITEF_SYSTEM32.txt" "SUCCESS" && echo: && echo Dlls Sitef atualizadas com sucesso em C:\Windows\System32
call :searchInFile "%LOGFILES%\TLS_SYSTEM32.txt" "SUCCESS" && echo Arquivo CONFITLS.ini configurado com sucesso em C:\Windows\System32

call :searchInFile "%LOGFILES%\UPDATE_DLLSITEF_LIBS.txt" "SUCCESS" && echo: && echo Dlls Sitef atualizadas com sucesso em C:\VRPdv\libs
call :searchInFile "%LOGFILES%\TLS_LIBS.txt" "SUCCESS" && echo Arquivo CONFITLS.ini configurado com sucesso em C:\VRPdv\libs

call :searchInFile "%LOGFILES%\TLS_PROPERTIES.txt" "SUCCESS" && echo: && echo Token TLS configurado no vr.properties com sucesso

call :searchInFile "%LOGFILES%\UPDATE_JAVA.txt" "SUCCESS" && echo: && echo Java Atualizado com sucesso
...
if exist "C:\Windows\SysWOW64\user32.dll" (
    set "firebirdFolder=%PROGRAMFILES(X86)%\Firebird"
) else (
    set "firebirdFolder=%PROGRAMFILES%\Firebird"
)

if exist "%firebirdFolder%\Firebird_2_5\unins000.exe" (
    echo.
    echo Removendo Firebird... Aguarde...
    echo Parando servicos Firebird...
    net stop "Firebird Guardian - DefaultInstance" >nul 2>&1
    net stop "Firebird Server - DefaultInstance" >nul 2>&1
    
	echo Iniciando Uninstaller Firebird...
    cd /d "%firebirdFolder%\Firebird_2_5"
    start "" /WAIT "unins000.exe" /SILENT
	timeout 2 >nul 2>&1
    
	echo Limpando arquivos residuais Firebird...
    rmdir /S /Q "%firebirdFolder%" >nul 2>&1
	timeout 2 >nul 2>&1
...
if exist "C:\Windows\SysWOW64\user32.dll" (
	...
	call :registerLog "UPDATE_DLLSITEF_SYSWOW64" "SUCCESS"
	explorer "C:\Windows\SysWOW64" >nul 2>&1
		...
		call :registerLog "UPDATE_DLLSITEF_SYSTEM32" "SUCCESS"
...
if exist "C:\VRPdv\libs" (
	...
	call :registerLog "UPDATE_DLLSITEF_LIBS" "SUCCESS"
...
call :registerLog "UPDATE_DLLSITEF" "SUCCESS"
...
REM FIREBIRD
rem set firebird_path32="C:\Program Files (x86)\Firebird"
rem cacls %firebird_path32% /E /T /C /P Todos:F REDE:F >nul 2>&1
...
set javacheck=%~2
   ...
   set javaInstallSucess=0
   if exist "C:\Program Files\Java" (
    set javaInstallSucess=1
	)
   if exist "C:\Program Files (x86)\Java" (
	set javaInstallSucess=1
	)

	if %javaInstallSucess% equ 0 (
	echo:
	echo ==================================================================
	echo ***** Falha na instalacao do Java, nenhuma pasta identificada ****
	echo ==================================================================
	pause
	exit /b
	)
	if %javaInstallSucess% equ 1 (
	call :checkJavaVersion
	echo:
	echo ===========================
	echo ***** Java com Sucesso ****
	echo %printjavamenu%
	echo ===========================
...
REM #################### settingVRProperties_NFCe ####################
:settingVRProperties_NFCe
echo:
set "file=C:\vr\vr.properties"

cacls C:\vr\vr.properties /E /T /C /P Todos:F REDE:F >nul 2>&1
cacls %userprofile%\.vr\vr.properties /E /T /C /P Todos:F REDE:F >nul 2>&1
call :datetime
copy C:\vr\vr.properties C:\vr\vr_%dateTime%.properties /Y >nul 2>&1

if not exist "C:\vr\nfe\certificado\certificado.pfx" (
	echo ================================================
	echo ATENCAO, FALHA.
	echo Arquivo C:\vr\nfe\certificado.pfx nao localizado
	echo ================================================
)
if not exist "C:\vr\nfe\certificado\certificado.jks" (
	echo ================================================
	echo ATENCAO, FALHA.
	echo Arquivo C:\vr\nfe\certificado.jks nao localizado
	echo ================================================
)

echo:
set /p "CSC_INFO=Informe o CSC: "
set /p "IDTOKEN_INFO=Informe o IDToken: "
set /p "SENHACERT_INFO=Informe a senha do Certificado: "

set "patterns=###################################################### #----------------------------------------------------- nfce.idtoken nfce.csc nfce.certificado.senha nfce.certificado.tipo nfce.tempoconexao nfce.tipoambiente nfce.certificado.diretorio nfce.diretorio nfce.enviaintermediador nfe.idtoken nfe.csc nfe.certificado.senha nfe.certificado.tipo nfe.tempoconexao nfe.tipoambiente nfe.certificado.diretorio nfe.diretorio repositorio.windows"

call :clearLinesFromFile "%file%" "%patterns%"

>> "%file%" echo ######################################################
>> "%file%" echo repositorio.windows = false
>> "%file%" echo nfce.idtoken=%IDTOKEN_INFO%
>> "%file%" echo nfce.csc=%CSC_INFO%
>> "%file%" echo nfce.certificado.senha=%SENHACERT_INFO%
>> "%file%" echo nfce.certificado.tipo=A1
>> "%file%" echo nfce.tempoconexao=30
>> "%file%" echo nfce.tipoambiente=1
>> "%file%" echo nfce.certificado.diretorio=C:\\vr\\nfe\\certificado\\
>> "%file%" echo nfce.diretorio=C:\\vr\\nfe\\
>> "%file%" echo nfce.enviaintermediador=true
>> "%file%" echo #-----------------------------------------------------
>> "%file%" echo nfe.idtoken=%IDTOKEN_INFO%
>> "%file%" echo nfe.csc=%CSC_INFO%
>> "%file%" echo nfe.certificado.senha=%SENHACERT_INFO%
>> "%file%" echo nfe.certificado.tipo=A1
>> "%file%" echo nfe.tempoconexao=30
>> "%file%" echo nfe.tipoambiente=1
>> "%file%" echo nfe.certificado.diretorio=C:\\vr\\nfe\\certificado\\
>> "%file%" echo nfe.diretorio=C:\\vr\\nfe\\
>> "%file%" echo ######################################################

echo Config NFCe no vr.properties finalizada
timeout 2
exit /b
...
>"%file%.tmp" (
    for /f "usebackq delims=" %%i in ("%file%") do (
        ...
        :: Remove tudo após o primeiro espaço ou =
        for /f "tokens=1 delims== " %%a in ("!line!") do set "firstToken=%%a"
        
        :: Verifica se o primeiro token está na lista de padrões
        for %%p in (%patterns%) do (
            if "!firstToken!"=="%%p" set "skip=true"
        )
        
        if not "!skip!"=="true" echo(!line!
    )
)
...
REM #################### checkProgramsFolder ####################
:checkProgramsFolder
if not exist "%PROGRAMS_FOLDER%" (
    cls
	echo:
	echo Atencao, falha
	echo Pasta Programs nao localizada em "%PROGRAMS_FOLDER%"
	echo Ela deve ficar junto ao Auto_Atualizador.bat
	pause
   exit
)
...
REM #################### checkProgramsFolder_deleteOlderFiles ####################
:checkProgramsFolder_deleteOlderFiles
if exist "%PROGRAMS_FOLDER%\wget" (
rmdir /S /Q "%PROGRAMS_FOLDER%\wget" >nul 2>&1
)
if exist "C:\pdv\util\Programs\wget" (
rmdir /S /Q "C:\pdv\util\Programs\wget" >nul 2>&1
)

set files="CliSiTef - 32.lnk" "CliSiTef - 64.lnk" "DllSitef.bat" "DllSitef_Copiar.bat" "DllSitef_Original.bat" "Auto_Atualizador_8.5.14" "Auto_Atualizador_8.5.15" "Auto_Atualizador_8.5.16" "Auto_Atualizador_8.5.17" "Auto_Atualizador_8.5.18" "Auto_Atualizador_9.0.bat" "Auto_Atualizador_9.1.bat" "Auto_Atualizador_9.2.bat"
set folders="%PROGRAMS_FOLDER%" "C:\pdv\util\Programs"
...
REM #################### deleteTempConfirmationFiles ####################
:deleteTempConfirmationFiles
if exist "%LOGFILES%" (
rmdir /S /Q "%LOGFILES%" >nul 2>&1
)
mkdir "%LOGFILES%" >nul 2>&1
...
echo %STATUS% > %LOGFILES%\%APPNAME%.txt
exit /b
REM #################### registerLog ####################
...
rem     echo ERRO: Arquivo nao encontrado: %file%
rem     exit /b 1
rem )

findstr /i /c:"%search_term%" "%file%" >nul 2>&1 && exit /b 0 || exit /b 1
REM #################### searchInFile ####################
REM =========================================================================
endlocal